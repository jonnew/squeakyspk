function RasterWave_Comp(SS, bound, Fs)
...
%
%   RASTERWAVE_COMP(SS, SS.time,chORun,SS.waveform) displays a clickable raster-plot of
...
%   SS.waveform matrix.
%
%   RASTERWAVE_COMP(SS,bound,Fs) creates a raster
%   wave plot for a portion of the full recording specified in bound = [start
%   stop] measured in whatever the units SS.time are in. Fs is the
%   sampling frequency of the waveforms provided. Default value is 25 KHz.
%
...
startind = find(sort(SS.time) > bound(1),1);
if bound(2) == SS.time(end)
    endind = length(SS.time);
else
    endind = find(sort(SS.time) > bound(2),1) - 1;
end
...
subplot(211)
hold on
plot(spkinterest(cleaninterest),unitinterest(cleaninterest),'w.','MarkerSize',3);
plot(spkinterest(~cleaninterest),unitinterest(~cleaninterest),'r.','MarkerSize',3); 
axis tight
h = gca;
set(h,'color','k','XColor',[1 1 1],'YColor',[1 1 1],'ylim',[0 max(unit)+1])
...
while but == 1
    ...
    if ~isempty(xi)
        ...
        subplot(212)
        t = 1000/Fs.*(1:size(SS.waveform,1));
        v = 1000.*SS.waveform(:,ind);
